...
type CompanyProduct struct {
	...
	MaterialOutUrl  string    `gorm:"column:material_out_url;type:text;comment:外部素材URL"`
	Commission      string    `gorm:"column:commission;type:text;comment:达人佣金机制"`
	InvestmentRatio float32   `gorm:"column:investment_ratio;type:decimal(10, 2) UNSIGNED;not null;default:0;comment:投流费比，单位：%"`
	ForbidReason    string    `gorm:"column:forbid_reason;type:varchar(250);not null;default:'';comment:下架原因"`
...
func (cpr *companyProductRepo) ListExternal(ctx context.Context, pageNum, pageSize int, industryId, categoryId, subCategoryId uint64, isInvestment uint8, productStatus, keyword string) ([]*domain.CompanyProduct, error) {
	...
	if industryId > 0 {
		db = db.Table("company_product FORCE INDEX (product_status_industry_id_category_id_sub_category_id)")
	}
...
func (cpr *companyProductRepo) Upsert(ctx context.Context, in *domain.DoukeProductGorm) error {
	if result := cpr.data.DB(ctx).Table("company_product").Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "product_out_id"}},
		DoUpdates: clause.AssignmentColumns([]string{"product_type", "product_name", "product_img", "product_price", "industry_id", "category_id", "sub_category_id", "shop_name", "total_sale", "commission_ratio", "update_time"}),

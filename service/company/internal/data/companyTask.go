...
type CompanyTask struct {
	...
	Price         uint64    `gorm:"column:price;type:int(10) UNSIGNED;not null;default:0;comment:每条价格"`
...
func (ctr *companyTaskRepo) GetByProductOutId(ctx context.Context, productOutId uint64) (*domain.CompanyTask, error) {
	task := &CompanyTask{}

	if err := ctr.data.db.WithContext(ctx).
		Where("product_out_id = ? and is_del = ?", productOutId, 0).First(task).Error; err != nil {
		return nil, err
	}
...
func (ctr *companyTaskRepo) List(ctx context.Context, pageNum, pageSize, isTop, isDel int, productOutIds []uint64) ([]*domain.CompanyTask, error) {
	...
	db := ctr.data.db.WithContext(ctx).Table("company_task")
...
func (ctr *companyTaskRepo) Save(ctx context.Context, in *domain.CompanyTask) (*domain.CompanyTask, error) {
	...
	if err := ctr.data.DB(ctx).Table("company_task").Create(task).Error; err != nil {
		return nil, err
	}
...
func (ctr *companyTaskRepo) UpdateCompanyTaskIsDel(ctx context.Context, id uint64) error {
	return ctr.data.DB(ctx).Table("company_task").Where("id = ?", id).Update("is_del", 1).Error
}
...
func (ctr *companyTaskRepo) CountByProjectOutId(ctx context.Context, productOutId uint64) (int64, error) {
	db := ctr.data.db.WithContext(ctx).Table("company_task").
		Where("product_out_id = ? and is_del = ?", productOutId, 0)
...
func (ctr *companyTaskRepo) ListIds(ctx context.Context) ([]int, error) {
	ids := []int{}

	if err := ctr.data.db.WithContext(ctx).Table("company_task").Select("id").
		Where("is_del = ?", 0).Scan(&ids).Error; err != nil {
		return ids, err
	}

...
func wireApp(confServer *conf.Server, confData *conf.Data, volcengine *conf.Volcengine, company *conf.Company, confService *conf.Service, registry *conf.Registry, event *conf.Event, logger log.Logger) (*kratos.App, func(), error) {
	...
	companyTaskAccountRelationUsecase := biz.NewCompanyTaskAccountRelationUsecase(companyTaskAccountRelationRepo, companyTaskRepo, companyTaskDetailRepo, companyProductRepo, doukeOrderRepo, transaction, confData, volcengine, logger)
	weixinUserRepo := data.NewWeixinUserRepo(dataData, logger)
	companyTaskDetailUsecase := biz.NewCompanyTaskDetailUsecase(companyTaskDetailRepo, companyTaskAccountRelationRepo, weixinUserRepo, confData, logger)

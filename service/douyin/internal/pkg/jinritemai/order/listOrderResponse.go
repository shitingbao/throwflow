...
type PidInfo struct {
	...
	ExternalInfo  string `json:"external_info"`   // 外部参数
	MediaTypeName string `json:"media_type_name"` // 分销类型，直播Live
}
...
type Order struct {
	...
	PidInfo                        PidInfo `json:"pid_info"`                   // 分销订单相关参数
...
func (lor *ListOrderResponse) Struct(response string) error {
	if err := json.Unmarshal([]byte(response), lor); err != nil {
		return jinritemai.NewJinritemaiError(90000, jinritemai.BaseDomain+"/alliance/kol/orders/", "解析json失败："+err.Error(), response)
	} else {
		if lor.CommonResponse.Extra.ErrorCode != 0 {
			return jinritemai.NewJinritemaiError(lor.CommonResponse.Extra.ErrorCode, jinritemai.BaseDomain+"/alliance/kol/orders/", lor.CommonResponse.Extra.Description, response)
		}
	}

...
type Menu struct {
	...
	MenuType       uint8     `gorm:"column:menu_type;type:tinyint(3) UNSIGNED;not null;default:0;comment:商家类型，1：目录，2：菜单，3：按钮"`
	Sort           uint8     `gorm:"column:sort;type:tinyint(3) UNSIGNED;not null;default:0;comment:菜单顺序 数值越小排位越靠前"`
	FileName       string    `gorm:"column:file_name;type:varchar(50);not null;comment:文件名[前端]"`
	IconName       string    `gorm:"column:icon_name;type:varchar(50);not null;comment:ICON[前端]"`
...
func (mr *menuRepo) ListByIds(ctx context.Context, ids []string) ([]*domain.Menu, error) {
	...
	if result := mr.data.db.WithContext(ctx).Order("sort ASC").Find(&menus, ids); result.Error != nil {
		return nil, result.Error
	}
